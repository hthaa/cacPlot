...
cacPlot <- function(x = NULL, ablty = NULL, ablty.se = NULL, pop.dist = c(0, 1), cutoff = 0, stat = "ca", ci = TRUE, cSEM = FALSE, xRng = c(-3, 3), yRng = c(0, .5), grid = TRUE, lbls = TRUE, rel.wdth = c(7, 1), mdl = "Rasch", ablty.est = "ML", colorblindFriendly = FALSE) {
  ...
  cac <- cacIRT::class.Rud(cutoff, ability = ab.est[, 1], se = ab.est[, 2], D = 1)$Conditional[[stat]]
  if (length(cutoff) > 1) {
    cac <- cac[, -(ncol(cac))]
  }
  ...
  if (ci) {
    ...
    apply(coords, 1, function(x) {
        lines(x[c(3, 4)], rep(x[5], 2), col = cacGradient(min(cacIRT::class.Rud(cutoff, D = 1, ability = x[1], se = x[2])$Conditional[[stat]][if (length(cutoff) > 1) { -(length(cutoff) + 1) } else { 1 }]),
                                                          colorblindFriendly), lwd = 2)
        lines(rep(x[3], 2), c(x[5] - (yRng[2] - yRng[1]) * .015, x[5] + (yRng[2] - yRng[1]) * .015),
              col = cacGradient(min(cacIRT::class.Rud(cutoff, D = 1, ability = x[1], se = x[2])$Conditional[[stat]][if (length(cutoff) > 1) { -(length(cutoff) + 1) } else { 1 }]), colorblindFriendly), lwd = 2)

...
#' @returns A list of diagnostic performance statistics, namely the Sensitivity, Specificity, Positive and Negative Likelihood Ratios (LR+ and LR-), Positive and Negative Predictive values (PPV and NPV), and the Youden's J statistic.
#' @export

dpstats <- function(cmat = NULL) {
    dp <- list()
    dp[["Sensitivity"]] <- cmat[1, 1] / (cmat[2, 2] + cmat[1, 1])
    dp[["Specificity"]] <- cmat[2, 1] / (cmat[2, 1] + cmat[1, 2])
    ...
    dp[["Youden. J"]] <- dp[["Sensitivity"]] + dp[["Specificity"]] -1
    return(dp)
}
